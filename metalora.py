...
Original file is located at
    https://colab.research.google.com/github/devdaveddev/Meta-LoRA/blob/main/MetaLoRa.ipynb
"""
...
# FIXED: Use the per_layer_results_by_threshold to get the correct structure
if 'per_layer_results_by_threshold' in globals() and 'chosen_energy' in globals():
     per_layer_out = per_layer_results_by_threshold[chosen_energy]
else:
    raise RuntimeError("per_layer_results_by_threshold or chosen_energy not found. Run SVD cell first.")
...
# === Task vs Storage Scaling ===
TASK_POINTS = [1, 5, 10, 20, 50, 100, 200, 500]
...
for i, row in df_final.iterrows():
    ...
    lora_storage = [row["lora_per_task_MB"] * T for T in TASK_POINTS]
    meta_exact_storage = [row["meta_shared_MB_exact"] + row["meta_per_task_MB_exact"] * T for T in TASK_POINTS]
    meta_drop_storage = [row["meta_shared_MB_drop"] + row["meta_per_task_MB_drop"] * T for T in TASK_POINTS]

    plt.plot(TASK_POINTS, lora_storage, "r--", label="LoRA" if i==0 else "")
    plt.plot(TASK_POINTS, meta_exact_storage, "b-", alpha=0.4, label=f"Meta-LoRA Exact (thr={thresh})" if i in [0,len(df_final)-1] else "")
    plt.plot(TASK_POINTS, meta_drop_storage, "g-", alpha=0.4, label=f"Meta-LoRA Drop (thr={thresh})" if i in [0,len(df_final)-1] else "")
